<?php
	...
	if($_POST['exec']=="add_group_ok") {
		...
		// 중복 이름 검사
		$check=mysql_fetch_array(zb_query("select count(*) from $group_table where name='$name'"));
	...
	elseif($_POST['exec']=="del_group_ok") {
		...
		// 멤버 이동
		if($member_move) {
			zb_query("update $member_table set group_no='$member_move' where group_no='$group_no'") or Error("회원 이동시에 에러가 발생하였습니다");
			zb_query("update $group_table set member_num=member_num+".$num['member_num']." where no='$member_move'");
		} else {
		...
		// 게시판이동
		if($board_move) {
			zb_query("update $admin_table set group_no='$board_move' where group_no='$group_no'") or Error("게시판 이동시에 에러가 발생하였습니다");
			zb_query("update $group_table set board_num=board_num+".$num['board_num']." where no='$board_move'");
		} else {
	...
	elseif($_POST['exec']=="modify_member_join_ok") {
		if(!isset($use_icq)) $use_icq=0;
		if(!isset($use_aol)) $use_aol=0;
		if(!isset($use_msn)) $use_msn=0;
		if(!isset($use_jumin)) $use_jumin=0;
		if(!isset($use_comment)) $use_comment=0;
		if(!isset($use_job)) $use_job=0;
		if(!isset($use_hobby)) $use_hobby=0;
		if(!isset($use_home_address)) $use_home_address=0;
		if(!isset($use_home_tel)) $use_home_tel=0;
		if(!isset($use_office_address)) $use_office_address=0;
		if(!isset($use_office_tel)) $use_office_tel=0;
		if(!isset($use_handphone)) $use_handphone=0;
		if(!isset($use_mailing)) $use_mailing=0;
		if(!isset($use_birth)) $use_birth=0;
		if(!isset($use_picture)) $use_picture=0;
